...
const App = () => {
    ...
    const apiKey ='live_kiPzybnTNZ5Vq1i1Q3miEeiAb27DrNlRKhBYBlCHfKWHodPxGT0WSqhTKiSpY3de'

    const url = `https://api.thecatapi.com/v1/images/search?limit=12&api_key=${apiKey}`
    ...
    const getCatFacts = () => {
        ...
        fetch('https://meowfacts.herokuapp.com/?count=12')
            .then(response => response.json())
            .then(data => {
                if (!ignore) {
            ...
            <h1 className='text-[50px] text-center font-head'>Kitty Facts :)</h1>

                {
                   catFacts.length > 0 ? <ul className='grid gap-8 grid-cols-[repeat(auto-fit,400px)]'>
                           {
                               catFacts.map((fact , i) => <li className='text-2xl text-center w-[75%] max-sm:w-[95%] max-sm:text-[20px] mx-auto mt-7' key={i}>
